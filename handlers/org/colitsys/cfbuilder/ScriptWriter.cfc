<cfcomponent output="false">
	...
	<cffunction name="init" access="public" returntype="ScriptWriter" output="false">
		<cfreturn this>
	</cffunction>
	...
	<cffunction name="processLine" access="public" returntype="string" output="false">
		...
		<cfif findNoCase("}",file) AND NOT findNoCase(";",file) AND NOT findNoCase("{",file)>
			<!--- format new lines after the end of a function --->
			<cfset file = replace(file,"}","}" & newline)>
		</cfif>

		<cfif findNoCase("	// application variables",file)>
			<cfset file = newline & "	// application variables">
		</cfif>
	...
	<cffunction name="replaceProperty" access="private" returntype="any" output="true">
		...
		<cfelse>
			<cfset file = reReplaceNoCase(file, 'this.#arguments.property# = {#arguments.property#};',"") />
		</cfif>
	...
	<cffunction name="cleanup" access="private" returntype="string" output="false">
		...
		<cfset file = reReplaceNoCase(file, ':component', 'component', 'all') />
		<!---- convert comments ---->
